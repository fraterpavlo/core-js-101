...
function willYouMarryMe(isPositiveAnswer) {
  return new Promise((resolve, reject) => {
    ...
    if (isPositiveAnswer !== true && isPositiveAnswer !== false) reject(new Error('Wrong parameter is passed! Ask her again.'));
  });
}
...
function chainPromises(array, action) {
  ...
  return new Promise((resolve) => {
    array.forEach(async (promise, idx) => {
      try {
        const resOfCurrentPromise = await promise;
        resolved.push(resOfCurrentPromise);
        if (idx === array.length - 1) resolve(resolved.reduce(action));
      } catch (err) {
        rejected.push(err);
      }
    });
